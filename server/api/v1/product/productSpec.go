...
func (prodSpecApi *ProductSpecApi) CreateProductSpec(c *gin.Context) {
    // 创建业务用Context
    ctx := c.Request.Context()

	var prodSpec product.ProductSpec
	err := c.ShouldBindJSON(&prodSpec)
	if err != nil {
		response.FailWithMessage(err.Error(), c)
		return
	}
	err = prodSpecService.CreateProductSpec(ctx,&prodSpec)
	if err != nil {
...
func (prodSpecApi *ProductSpecApi) DeleteProductSpec(c *gin.Context) {
    // 创建业务用Context
    ctx := c.Request.Context()

	ID := c.Query("ID")
	err := prodSpecService.DeleteProductSpec(ctx,ID)
	if err != nil {
...
// @Router /prodSpec/deleteProductSpecByIds [delete]
func (prodSpecApi *ProductSpecApi) DeleteProductSpecByIds(c *gin.Context) {
    // 创建业务用Context
    ctx := c.Request.Context()

	IDs := c.QueryArray("IDs[]")
	err := prodSpecService.DeleteProductSpecByIds(ctx,IDs)
	if err != nil {
...
func (prodSpecApi *ProductSpecApi) UpdateProductSpec(c *gin.Context) {
	...
	err = prodSpecService.UpdateProductSpec(ctx,prodSpec)
	if err != nil {
...
func (prodSpecApi *ProductSpecApi) FindProductSpec(c *gin.Context) {
    // 创建业务用Context
    ctx := c.Request.Context()

	ID := c.Query("ID")
	reprodSpec, err := prodSpecService.GetProductSpec(ctx,ID)
	if err != nil {
        global.GVA_LOG.Error("查询失败!", zap.Error(err))
		response.FailWithMessage("查询失败:" + err.Error(), c)
		return
	}
	response.OkWithData(reprodSpec, c)
}
...
func (prodSpecApi *ProductSpecApi) GetProductSpecList(c *gin.Context) {
    // 创建业务用Context
    ctx := c.Request.Context()
	...
	list, total, err := prodSpecService.GetProductSpecInfoList(ctx,pageInfo)
	if err != nil {
...
func (prodSpecApi *ProductSpecApi) GetProductSpecPublic(c *gin.Context) {
    // 创建业务用Context
    ctx := c.Request.Context()

    // 此接口不需要鉴权
    // 示例为返回了一个固定的消息接口，一般本接口用于C端服务，需要自己实现业务逻辑
    prodSpecService.GetProductSpecPublic(ctx)
