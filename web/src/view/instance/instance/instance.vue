...
<template>
  ...
  <el-select v-model="searchInfo.serverId" filterable placeholder="请选择节点" :clearable="false">
  ...
  <el-select v-model="searchInfo.templateId" filterable placeholder="请选择模版" :clearable="false">
  ...
  <el-select v-model="searchInfo.imageId" filterable placeholder="请选择镜像" :clearable="false">
    <el-option v-for="(item,key) in dataSource.imageId" :key="key" :label="item.label" :value="item.value" />
  </el-select>
</el-form-item>
            
            <el-form-item label="Docker容器ID" prop="containerId">
  ...
  <el-input v-model="searchInfo.name" placeholder="搜索条件" />
    ...
    <el-tree-select v-model="searchInfo.status" placeholder="请选择状态" :data="instance_statusOptions" style="width:100%" filterable :clearable="false" check-strictly ></el-tree-select>
</el-form-item>
  ...
  <el-input v-model="searchInfo.remark" placeholder="搜索条件" />
    ...
    <template #default="scope">
        <span>{{ filterDataSource(dataSource.userId,scope.row.userId) }}</span>
            ...
            <el-table-column align="left" label="节点" prop="serverId" width="120">
    <template #default="scope">
        <span>{{ filterDataSource(dataSource.serverId,scope.row.serverId) }}</span>
    </template>
</el-table-column>
    ...
    <template #default="scope">
        <span>{{ filterDataSource(dataSource.imageId,scope.row.imageId) }}</span>
            ...
            <el-table-column align="left" label="Docker容器ID" prop="containerId" width="120" />

            <el-table-column align="left" label="实例名称" prop="name" width="120" />

            <el-table-column align="left" label="状态" prop="status" width="120">
    <template #default="scope">
    {{ filterDict(scope.row.status,instance_statusOptions) }}
    </template>
</el-table-column>
    ...
    <el-select v-model="formData.templateId" placeholder="请选择模版" filterable style="width:100%" :clearable="false">
        <el-option v-for="(item,key) in dataSource.templateId" :key="key" :label="item.label" :value="item.value" />
            ...
            <el-form-item label="镜像:" prop="imageId">
    <el-select v-model="formData.imageId" placeholder="请选择镜像" filterable style="width:100%" :clearable="false">
        <el-option v-for="(item,key) in dataSource.imageId" :key="key" :label="item.label" :value="item.value" />
    </el-select>
</el-form-item>
            <el-form-item label="实例名称:" prop="name">
    <el-input v-model="formData.name" :clearable="true" placeholder="请输入实例名称" />
            ...
            <el-form-item label="状态:" prop="status">
    <el-tree-select v-model="formData.status" placeholder="请选择状态" :data="instance_statusOptions" style="width:100%" filterable :clearable="false" check-strictly />
</el-form-item>
            <el-form-item label="备注:" prop="remark">
    ...
    <template #default="scope">
        <span>{{ filterDataSource(dataSource.serverId,detailForm.serverId) }}</span>
    </template>
</el-descriptions-item>
                    <el-descriptions-item label="模版">
    <template #default="scope">
        <span>{{ filterDataSource(dataSource.templateId,detailForm.templateId) }}</span>
    </template>
</el-descriptions-item>
    ...
    <template #default="scope">
        <span>{{ filterDataSource(dataSource.imageId,detailForm.imageId) }}</span>
                    ...
                    <el-descriptions-item label="Docker容器ID">
