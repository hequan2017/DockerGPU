...
<template>
  ...
  <el-input v-model="searchInfo.address" placeholder="搜索条件" />
    ...
    <el-tree-select v-model="searchInfo.type" placeholder="请选择类型" :data="registry_typeOptions" style="width:100%" filterable :clearable="true" check-strictly ></el-tree-select>
</el-form-item>
            
            <el-form-item label="是否上架" prop="shelfStatus">
              <el-select v-model="searchInfo.shelfStatus" placeholder="请选择" style="width:100%" :clearable="true">
  ...
  <el-input v-model="searchInfo.remark" placeholder="搜索条件" />
    ...
    <div class="gva-table-box">
        <div class="gva-btn-list">
            ...
            <ExportTemplate  template-id="registry_Registry" />
            <ExportExcel  template-id="registry_Registry" filterDeleted/>
            <ImportExcel  template-id="registry_Registry" @on-success="getTableData" />
        </div>
        <el-table
        ref="multipleTable"
        style="width: 100%"
    ...
    {{ filterDict(scope.row.type,registry_typeOptions) }}
    </template>
</el-table-column>
            <el-table-column align="left" label="是否上架" prop="shelfStatus" width="120">
    <template #default="scope">
      {{ formatBoolean(scope.row.shelfStatus) }}
    </template>
  </el-table-column>
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            <template #default="scope">
            <el-button  type="primary" link class="table-button" @click="getDetails(scope.row)"><el-icon style="margin-right: 5px"><InfoFilled /></el-icon>查看</el-button>
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateRegistryFunc(scope.row)">编辑</el-button>
...
const getTableData = async() => {
  const table = await getRegistryList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteRegistryByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateRegistryFunc = async(row) => {
    const res = await findRegistry({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteRegistryFunc = async (row) => {
    const res = await deleteRegistry({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
